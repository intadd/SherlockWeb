...
def report(request):
  ...
  times=timezone.now()
  if username:
    p=userLog(userName=username,ipLog=user_ip,UserAgent=user_agent,created_date=times,author_id=1)
    p.save()
    urls=[]
...
def sherlock_api(request):
 ...
 if (tasks_key):
    ...
    if(res.status =='SUCCESS'):
        ...
        urls=datas[1]
        if(res.status !='NO'):
            urls=urls.replace('&','&amp;').replace('<','&lt;').replace('>','&gt;').replace('(','&#40;').replace('"','&quot').replace('\'','&#x27').replace('\\','&#x2F').replace(')','&#41;')
